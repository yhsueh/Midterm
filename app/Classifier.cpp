...
void Classifier::get_svm_detector()
{
    ...
    std::memcpy( &primalSupportVector[0], sv.ptr(), sv.cols*sizeof( primalSupportVector[0] ) );
    primalSupportVector[sv.cols] = (float)-rho;
}
...
bool Classifier::predictImg(std::string streetPath){
   ...
   // cv::waitKey();
    ...
    for (size_t i = 0; i < detections.size(); i++){
        if (foundWeights[i] > 1 && detections[i].y > 140){ //&& foundWeights[i] <3.5
